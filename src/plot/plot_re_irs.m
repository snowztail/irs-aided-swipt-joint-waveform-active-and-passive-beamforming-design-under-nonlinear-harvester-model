...
reLinearIrsSet = cell(nBatches, length(Variable.bandwidth));
...
for iBatch = 1 : nBatches
    try
		load(sprintf('../data/re_irs/re_irs_%d.mat', iBatch), 'reIdealIrsInstance', 'reAdaptiveIrsInstance', 'reLinearIrsInstance', 'reWitIrsInstance', 'reWptIrsInstance', 'reRandomIrsInstance', 'reNoIrsInstance');
        reIdealIrsSet(iBatch, :) = reIdealIrsInstance;
        reAdaptiveIrsSet(iBatch, :) = reAdaptiveIrsInstance;
        reLinearIrsSet(iBatch, :) = reLinearIrsInstance;
...
reLinearIrs = cell(1, length(Variable.bandwidth));
...
for iBandwidth = 1 : length(Variable.bandwidth)
	...
	reLinearIrs{iBandwidth} = mean(cat(3, reLinearIrsSet{indexSet, iBandwidth}), 3);
