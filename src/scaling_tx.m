...
for iChannel = 1 : nChannels
    for iTx = 1 : length(Variable.nTxs)
		...
		% * WIT
		[rateSample(iChannel, iTx), irs, infoAmplitude, powerAmplitude, infoRatio, powerRatio] = wit(directChannel, incidentChannel, reflectiveChannel, txPower, noisePower, nCandidates, tolerance);
		[compositeChannel] = composite_channel(directChannel, incidentChannel, reflectiveChannel, irs);
		witSolution{iChannel, iTx} = variables2struct(irs, compositeChannel, infoAmplitude, powerAmplitude, infoRatio, powerRatio);
		clearvars irs compositeChannel infoAmplitude powerAmplitude infoRatio powerRatio;

		% * WPT by linear harvester model
		[currentLinearSample(iChannel, iTx), irs, infoAmplitude, powerAmplitude, infoRatio, powerRatio] = wpt_linear(beta2, beta4, directChannel, incidentChannel, reflectiveChannel, txPower, noisePower, nCandidates, tolerance);
		[compositeChannel] = composite_channel(directChannel, incidentChannel, reflectiveChannel, irs);
		wptLinearSolution{iChannel, iTx} = variables2struct(irs, compositeChannel, infoAmplitude, powerAmplitude, infoRatio, powerRatio);
		clearvars irs compositeChannel infoAmplitude powerAmplitude infoRatio powerRatio;

		% * WPT by nonlinear harvester model
		[currentNonlinearSample(iChannel, iTx), irs, infoAmplitude, powerAmplitude, infoRatio, powerRatio] = wpt(beta2, beta4, directChannel, incidentChannel, reflectiveChannel, txPower, noisePower, nCandidates, tolerance);
		[compositeChannel] = composite_channel(directChannel, incidentChannel, reflectiveChannel, irs);
		wptNonlinearSolution{iChannel, iTx} = variables2struct(irs, compositeChannel, infoAmplitude, powerAmplitude, infoRatio, powerRatio);
		clearvars irs compositeChannel infoAmplitude powerAmplitude infoRatio powerRatio;
    end
end
