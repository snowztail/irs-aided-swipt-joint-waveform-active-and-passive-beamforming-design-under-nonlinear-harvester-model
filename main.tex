...
\begin{document}
	...
	\begin{section}{Problem Formulation}\label{se:problem_formulation}
		...
		\begin{subsection}{Passive Beamforming}
			...
			\begin{algorithm}[!t]
				...
				\begin{algorithmic}[1]
					...
					\Repeat
						\State $i \gets i + 1$
						\State Get $\boldsymbol{\Phi}^{(i)},t_{\mathrm{I/P},k}^{(i)}$, $\forall k$ by solving \ref{ob:irs} -- \ref{co:irs_sd}
		...
		\begin{subsection}{Block Coordinate Descent}
			For any direct, incident and reflective channels, we iteratively update the passive beamforming $\boldsymbol{\phi}$ by Algorithm~\ref{al:irs}, and the waveform, active beamforming $\boldsymbol{w}_{\mathrm{I/P}}$ together with the splitting ratio $\rho$ by Algorithm~\ref{al:waveform} until convergence. The BCD algorithm is summarized in Algorithm~\ref{al:alternating}.
			\begin{algorithm}[!t]
				\caption{BCD: Waveform, Active and Passive Beamforming, and Splitting Ratio.}
				...
				\begin{algorithmic}[1]
					...
					\Repeat
						\State $i \gets i + 1$
						\State Fix $\boldsymbol{w}_{\mathrm{I/P}}^{(i-1)},\rho^{(i-1)}$, get $\boldsymbol{\phi}^{(i)}$ by Algorithm~\ref{al:irs}
