...
\begin{document}
	...
	\begin{section}{Problem Formulation}\label{se:problem_formulation}
		...
		\begin{remark}
			At subband $n$, the MIMO incident channel $\boldsymbol{H}_{I,n}$ can be divided into $L$ MISO channels between all transmit antennas and each IRS element. Reflection coefficient $\phi_l$ is shared for the $l$-th MISO incoming channel of all subbands, namely $MN$ entries in total. As information and power transfer favor different channel strength distribution over frequencies, for a given channel, the passive beamforming need to be optimized for each R-E point to maximize the R-E tradeoff. If the IRS is frequency-selective and there is only one transmit antenna, then reflector $l$ can simply align the AP-IRS($l$)-user channel with the AP-user channel at all subbands so that the joint design of waveform, active and passive beamforming, and splitting ratio would follow straightforwardly from existing SWIPT work \cite{Clerckx2018b}.
		\end{remark}
	...
	\begin{section}{Performance Evaluations}\label{se:performance_evaluation}
		...
		On the other hand, the rightmost point corresponds to WIT ($\rho=0$) where the solution coincides with the Water-Filling (WF) algorithm that allocates all power to modulated waveform only. For a fair comparison, the $x$-axis of the plots has been normalized to per-subband rate.
