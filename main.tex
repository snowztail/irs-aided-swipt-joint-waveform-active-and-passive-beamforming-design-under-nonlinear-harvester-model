...
\begin{section}{Performance Evaluations}
	...
	% N increase -> more to balance, tradeoff between subbands
	...
	\begin{figure}
		...
		\end{adjustbox}
		\caption{Average R-E region versus the average noise power}
		\label{fi:re_noise}
	\end{figure}

	The influence of the average noise power on the average R-E region is investigated in Fig. \ref{fi:re_noise}. It is noted that for a large number of subbands ($N=16$ in this case), the R-E region is approximately concave for a high noise level and approximately convex for a low noise level. Moreover, there exists a

	% This is because

	As a result, TS is preferred at low SNR while PS is preferred at high SNR.

	% sections -> waterfilling: \rho only vs joint

	% irs, low noise power, high received power  L+->high snr, rho vs other L, M: snr and nonlinearity
	...
	\begin{figure}
		\centering
		\begin{adjustbox}{width=\linewidth}
			\input{assets/re_los.tex}
		\end{adjustbox}
		\caption{Average R-E region for IRS-aided NLoS and LoS channels}
		\label{fi:re_los}
	\end{figure}
