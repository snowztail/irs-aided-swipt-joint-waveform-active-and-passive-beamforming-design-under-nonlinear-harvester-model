...
\begin{abstract}
	Simultaneous Wireless Information and Power Transfer (SWIPT) is limited by the low power level of the receive Radio-Frequency (RF) signal. To tackle this problem, we introduce Intelligent Reflecting Surface (IRS) that brings a high passive beamforming gain by adjusting reflecting elements, which compensates the propagation loss and boosts the energy efficiency with low power consumption. This paper investigates an IRS-aided multiuser Orthogonal Frequency Division Multiplexing (OFDM) SWIPT system based on practical nonlinear harvester model, where the single-antenna Access Point (AP) transmits information and energy simultaneously to multiple single-antenna users under the assist of IRS. We aim to maximize the weighted sum Rate-Energy (R-E) region via jointly optimizing the transmit waveform at the AP, the phase shifts at the IRS, and the power splitting ratio at all users. The problem is transformed to a current maximization problem subject to rate constraint, and we propose an low-complexity alternating algorithm to obtain suboptimal solutions iteratively. Simulation results demonstrated significant R-E region enlargement over benchmark schemes when IRS is properly configured.
\end{abstract}
...
\begin{section}{Single-User Optimization}
	...
	\begin{maxi!}
		...
		\end{maxi!}
	Problem \ref{op:su} is intricate due to the non-convex objective function with coupled variables. To reduce the design complexity, we propose an suboptimal alternating optimization algorithm that iteratively updates the IRS phase shift, transmit waveform, and receive power splitting ratio iteratively.
	% In the next section, we propose waveform and phase shift design algorithms for frequency-flat and frequency-selective IRS, respectively.
	...
	\begin{subsection}{IRS Phase Shift}
		\begin{subsubsection}{Frequency-Selective IRS}\label{se:fs_irs}
			...
			\end{equation}
			That is to say, the optimal phase shift is obtained in closed form in the single-user scenario, and the phase of the composite channel equals that of the direct channel.
		\end{subsubsection}
		...
		\begin{subsubsection}{Frequency-Flat IRS}
			...
			\end{equation}
			where $\boldsymbol{M}=[\boldsymbol{V}^H,\boldsymbol{h}_D]^H \in \mathbb{C}^{(L+1) \times N}$.
			...
			\begin{algorithm}
				...
				\label{al:ff_irs}
				\begin{algorithmic}[1]
					\State \textbf{Input} $\beta_2,\beta_4,\boldsymbol{h}_D,\boldsymbol{h}_I,\boldsymbol{h}_R,Q,\bar{R},\epsilon,\rho,\boldsymbol{w}_{I/P},\sigma_n \ \forall n$ by \ref{eq:R_n,phi}, \ref{eq:C_{I/P,n}}
	...
	\begin{subsection}{Waveform and Splitting Ratio}
		\begin{subsubsection}{Geometric Programming}
			Following \cite{Clerckx2018b}, it can be observed from \ref{eq:R_k} and \ref{eq:z_k_expand} that the optimal phases of information and power waveform are both match to the composite channel as
			...
			\begin{equation}\label{eq:s}
				s_{I/P,n} = \lvert w_{I/P,n} \rvert e^{j\psi_{I/P,n}}
			\end{equation}
		...
		\begin{subsubsection}{Semi-Definite Relaxation}
			...
			\begin{paragraph}{Transmit Waveform}
				...
				\end{maxi!}
				We then perform SDR and solve the optimal waveform matrix $\boldsymbol{W}_{I/P}^{\star}$ iteratively by interior-point method. $\boldsymbol{w}_I^{\star}$ and $\boldsymbol{w}_P^{\star}$ can also be extracted using randomized vectors $\boldsymbol{r}_q \in \mathbb{C}^{(L+1) \times 1}, \ q=1,\dots,Q$, whose entries are uniformly distributed on the unit circle $r_{q,n}=e^{j\xi}$, $\xi \sim \mathcal{U}[0,2\pi)$. The algorithm is summarized in Algorithm \ref{al:waveform_sdr}.
				\begin{algorithm}
					\caption{SDR: Transmit Waveform}
					\label{al:waveform_sdr}
					\begin{algorithmic}[1]
						\State \textbf{Input} $\beta_2,\beta_4,\boldsymbol{h}_D,\boldsymbol{h}_I,\boldsymbol{h}_R,P,Q,\bar{R},\epsilon,\rho,\sigma_n \ \forall n$ by \ref{eq:R_n,phi}
			...
			\begin{paragraph}{Receive Splitting Ratio Optimization}
	...
	\begin{subsection}{R-E Region Characterization}
		\begin{subsubsection}{WIT Initialization}
			...
			\begin{paragraph}{FS-IRS}
				As discussed in Section \ref{se:fs_irs}, the optimal FS-IRS and composite channel are obtained in closed form. We use water-filling algorithm and matched filter to obtain optimal power allocation $p_n^\star=\lvert w_{I,n} \rvert^2$ and information waveform phase $\psi_{I,n}^\star=-\bar{\psi}_n$, respectively. Hence, the optimal information waveform at subband $n$ is
		...
		\begin{subsubsection}{R-E Sample}
			Denote $S$ as the number of samples in the R-E boundary.
			\begin{paragraph}{FS-IRS}
				Since the optimal composite channel is fixed, we optimize the splitting ratio and waveform alternatively with the other being fixed. The procedure is included in Algorithm \ref{al:fs_ao}.
				\begin{algorithm}
					...
					\begin{algorithmic}[1]
						...
						\Repeat
						\State $i \gets i + 1$
						\State Update splitting ratio $\rho_s^{(i)}$ by solving problem \ref{op:su_ratio}
						\State Update waveform $\boldsymbol{w}_{s,I/P}^{(i)}$ by Algorithm \ref{al:waveform_sdr}
						\Until $\lvert (z_s^{(i)}-z_s^{(i-1)}) / z_s^{(i)} \rvert \le \epsilon$\label{st:inner_end}
			...
			\begin{paragraph}{FF-IRS}
				We propose a two-layer AO algorithm to obtain the R-E region for FF-IRS-assisted SWIPT, where the inner loop updates the splitting ratio and waveform while the outer loop updates the IRS phase shifts, until convergence is achieved. The algorithm is summarized in Algorithm \ref{al:ff_ao}.
				\begin{algorithm}
					...
					\begin{algorithmic}[1]
						...
						\State \textit{Inner loop}: Update splitting ratio $\rho_s^{(0)}$ and waveform $\boldsymbol{w}_{s,I/P}^{(0)}$ by SDR (Algorithm \algref{al:fs_ao}{st:inner_begin} -- \algref{al:fs_ao}{st:inner_end}) or GP (Algorithm \ref{al:waveform_split_ratio_gp})
						\Repeat
						\State $i \gets i + 1$
						...
						\State \textit{Inner loop}: Update splitting ratio $\rho_s^{(i)}$ and waveform $\boldsymbol{w}_{s,I/P}^{(i)}$ by SDR (Algorithm \algref{al:fs_ao}{st:inner_begin} -- \algref{al:fs_ao}{st:inner_end}) or GP (Algorithm \ref{al:waveform_split_ratio_gp})
