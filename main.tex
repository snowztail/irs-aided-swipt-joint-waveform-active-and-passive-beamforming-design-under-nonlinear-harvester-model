...
\begin{document}
	...
	\begin{section}{Problem Formulation}\label{se:problem_formulation}
		...
		\begin{subsection}{Low-Complexity Adaptive Design}
			\begin{subsubsection}{Waveform and Active Beamforming}
				...
				\begin{align}
					s_{\mathrm{I}, n} &= \sqrt{2(1 - \delta)\left(\mu - \frac{\sigma_n}{P \lVert{\boldsymbol{h}_n}\rVert^2}\right)^+} \\
	...
	\begin{section}{Performance Evaluations}\label{se:performance_evaluation}
		...
		\begin{subsection}{Imperfect CSIT}
			Due to the general lack of RF-chains at the IRS, accurate CSIT acquisition can be challenging especially when the coherence time is short. Hence, we consider an imperfect CSIT model and denote the estimation of direct and cascaded links at subband $n$ as
			\begin{align}
				\hat{\boldsymbol{h}}_{\mathrm{D},n} &= \boldsymbol{h}_{\mathrm{D},n} + \tilde{\boldsymbol{h}}_{\mathrm{D},n} \\
				\hat{\boldsymbol{V}}_{n} &= \boldsymbol{V}_{n} + \tilde{\boldsymbol{V}}_{n}
			\end{align}
			where $\tilde{\boldsymbol{h}}_{\mathrm{D},n}$ and $\tilde{\boldsymbol{V}}_{n}$ are the corresponding estimation errors, whose entries follow i.i.d. CSCG distribution $\mathcal{CN}(0, \epsilon_{\mathrm{D},n}^2)$ and $\mathcal{CN}(0, \epsilon_{n}^2)$, respectively. Perfect CSIT can be regarded as a special case with $\epsilon_{\mathrm{D},n} = \epsilon_{n} = 0$. % For each direct and cascaded channel, we provide \num{100} channel estimates and examine the average R-E behavior.
		\end{subsection}

		\begin{subsection}{Quantized IRS}
			Recall in Remark \ref{re:reflection_coefficient} that one needs to vary the element impedance to adjust the reflection coefficient. To reduce the circuit complexity and control overhead, we design a passive beamforming codebook $\mathcal{C}_\phi = \{e^{j 2 \pi i / 2^b} \mid i = 1, \dots, 2^b\}$ and quantize each reflection coefficient by $b$ bits.
		\end{subsection}
		...
		\begin{figure}[!t]
			...
			\subfloat[Waveform amplitude\label{fi:waveform_subband}]{
				\resizebox{0.45\columnwidth}{!}{
					\input{assets/waveform_subband.tex}
				}
			}
			\caption{Average R-E region and waveform amplitude versus $N$ for $M=1$, $L=20$, $\sigma_n=-40\,\si{\dBm}$, $B=1\,\si{\MHz}$ and $d_{\mathrm{H}}=d_{\mathrm{V}}=2\,\si{\meter}$.}
		\end{figure}
		...
		\begin{figure}[!t]
			...
			\caption{Average R-E region and path loss versus $d_{\mathrm{H}}$ for $M=1$, $N=16$, $L=20$, $\sigma_n=-40\,\si{\dBm}$, $B=1\,\si{\MHz}$ and $d_{\mathrm{V}}=2\,\si{\meter}$.}
		\end{figure}
		...
		\begin{figure}[!t]
			...
			\caption{Average R-E region, WIT SNR and WPT DC current versus $M$ for $N=16$, $L=20$, $\sigma_n=-40\,\si{\dBm}$, $B=1\,\si{\MHz}$, $d_{\mathrm{H}}=d_{\mathrm{V}}=0.5\,\si{\meter}$.}
		\end{figure}
		...
		\begin{figure}[!t]
			...
			\caption{Average R-E region, WIT SNR and WPT DC current versus $L$ for $M=1$, $N=16$, $\sigma_n=-40\,\si{\dBm}$, $B=1\,\si{\MHz}$ and $d_{\mathrm{H}}=d_{\mathrm{V}}=0.5\,\si{\meter}$.}
		\end{figure}
		...
		\begin{figure}[!t]
			...
			\caption{Average R-E region for ideal, adaptive, fixed and no IRS versus $B$ for $M=1$, $N=16$, $L=20$, $\sigma_n=-40\,\si{\dBm}$ and $d_{\mathrm{H}}=d_{\mathrm{V}}=2\,\si{\meter}$.}
		\end{figure}
