...
\begin{document}
	...
	\begin{section}{System Model}\label{se:system_model}
		...
		\begin{subsection}{Composite Channel}
			...
			\begin{remark}
				The IRS reflection coefficients are constant across the frequency, i.e. the IRS is frequency-flat where independent phase shift control over subbands are prohibited due to hardware constraints.
			\end{remark}
		\end{subsection}
		...
		\begin{subsection}{Energy Harvester}
			...
			% 			\left(
			% 				\begin{array}{cccc}
			% 					\cellcolor[gray]{0.5}\boldsymbol{w}_{I/P,1}\boldsymbol{w}_{I/P,1}^H & \cellcolor[gray]{0.7}\boldsymbol{w}_{I/P,1}\boldsymbol{w}_{I/P,2}^H & \dots                                                                 & \cellcolor[gray]{0.9}\boldsymbol{w}_{I/P,1}\boldsymbol{w}_{I/P,N}^H   \\
			% 					\cellcolor[gray]{0.7}\boldsymbol{w}_{I/P,2}\boldsymbol{w}_{I/P,1}^H & \cellcolor[gray]{0.5}\boldsymbol{w}_{I/P,2}\boldsymbol{w}_{I/P,2}^H & \cellcolor[gray]{0.7}\ddots                                           & \vdots                                                                \\
			% 					\vdots                                                              & \cellcolor[gray]{0.7}\ddots                                         & \cellcolor[gray]{0.5}\ddots                                           & \cellcolor[gray]{0.7}\boldsymbol{w}_{I/P,N-1}\boldsymbol{w}_{I/P,N}^H \\
			% 					\cellcolor[gray]{0.9}\boldsymbol{w}_{I/P,N}\boldsymbol{w}_{I/P,1}^H & \dots                                                               & \cellcolor[gray]{0.7}\boldsymbol{w}_{I/P,N}\boldsymbol{w}_{I/P,N-1}^H & \cellcolor[gray]{0.5}\boldsymbol{w}_{I/P,N}\boldsymbol{w}_{I/P,N}^H
			% 				\end{array}
			% 			\right)$
			% 		}
			% 	\end{equation*}
			% 	\caption{$\boldsymbol{W}_{I/P,n}$ keeps the $n$-th block diagonal of $\boldsymbol{W}_{I/P}$ and nulls the remaining entries. Solid, dashed and dotted diagonals correspond to $n>,=,<0$. For $\boldsymbol{w}_{I/P,n_1}\boldsymbol{w}_{I/P,n_2}^H$, the $n$-th diagonal satisfies $n_2-n_1=n$.}
			...
			\begin{figure*}[!t]
				\begin{equation*}
						...
						\end{tikzpicture}
				\end{equation*}
				\caption{$\boldsymbol{W}_{I/P}$ consists of $N \times N$ blocks of size $M \times M$. $\boldsymbol{W}_{I/P,k}$ keeps the $k$-th block diagonal of $\boldsymbol{W}_{I/P}$ and nulls all remaining blocks. Solid, dashed and dotted blocks correspond to $n>0$, $n=0$ and $n<0$, respectively. For $\boldsymbol{w}_{I/P,n_1}\boldsymbol{w}_{I/P,n_2}^H$, the $k$-th block diagonal satisfies $k=n_2-n_1$.}
				\label{fi:block_diagonal}
			\end{figure*}

			On top of this, we define $\boldsymbol{W}_{I/P}=\boldsymbol{w}_{I/P}\boldsymbol{w}_{I/P}^H \in \mathbb{C}^{MN \times MN}$ and let $\boldsymbol{W}_{I/P,n} \in \mathbb{C}^{MN \times MN}$ keep their $n$-th ($n=-N+1,\dots,N-1$) block diagonal and set all other blocks to $\boldsymbol{0}^{M \times M}$.

			null the remaining entries, where the blocks are of size $M \times M$. On top of this, relevant DC terms are expressed in \eqref{eq:y_I2} -- \eqref{eq:y_P4}.
			\begin{figure*}[b]
				\hrule
				\begin{equation}
	...
	\begin{section}{Problem Formulation}\label{se:problem_formulation}
		...
		\begin{subsection}{IRS Phase Shift}
			...
			\begin{algorithm}[!t]
				...
				\begin{algorithmic}[1]
					...
					\Repeat
						\State $i \gets i + 1$
						\State Obtain $\boldsymbol{\Phi}^{(i)}, t_{I/P,n}^{(i)}$ by solving problem~\eqref{op:irs}
		...
		\begin{subsection}{Waveform and Splitting Ratio}
			...
			\end{mini!}
			The denominators of \eqref{co:waveform_objective}, \eqref{co:waveform_rate} are posynomials \cite{Boyd2007} that further decompose as
			\begin{align}
