...
\begin{section}{Performance Evaluations}\label{se:performance_evaluation}
	...
	\begin{figure}
		...
		\end{adjustbox}
		\caption{Average R-E region for adaptive, fixed and no IRS over $B = 10$ \si{\MHz}}
		\label{fi:re_irs}
	\end{figure}

	% Same reflection coefficient, When b small, even if n large, behaves as if ff
	% FF channel -> closed form optimal IRS; FS channel -> tradeoff between subbands
	% \textit{Third}, although each IRS element has the same reflection coefficient for $N$ subbands, increasing $N$ does not necessarily mean
	% frequency selective less multipath loss less with FF IRS
