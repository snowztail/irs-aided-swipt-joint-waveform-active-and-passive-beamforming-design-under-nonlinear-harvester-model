...
\begin{section}{Problem Formulation}
	...
	\begin{maxi!}
		{\boldsymbol{w}_I,\boldsymbol{w}_P,\boldsymbol{\phi},\rho}{z(\boldsymbol{w}_I,\boldsymbol{w}_P,\boldsymbol{\phi},\rho)}{\label{op:original}}{\label{eq:su_target}}
		\addConstraint{\frac{1}{2}({\boldsymbol{w}_I^H}{\boldsymbol{w}_I}+{\boldsymbol{w}_P^H}{\boldsymbol{w}_P})\le{P}}
		\addConstraint{R \ge \bar{R}}\label{eq:su_rate_constraint}
		\addConstraint{\lvert{\phi_l}\rvert=1, \quad l=1,\dots,L}
		\addConstraint{0 \le \rho \le 1}
	\end{maxi!}
	Problem \ref{op:original} is intricate with coupled variables involved in non-convex objective function \ref{eq:su_target} and rate constraint \ref{eq:su_rate_constraint}. To reduce the design complexity, we propose an suboptimal AO algorithm that iteratively updates the IRS phase shift and transmit waveform together with receive splitting ratio until convergence.
	...
	\begin{subsection}{IRS Phase Shift}
		...
		To maximize non-concave expression \ref{eq:z_irs}, we use first-order Taylor expansion to approximate its second-order terms. Based on the variables optimized at iteration $i - 1$, the local approximation at iteration $i$ suggests \cite{Adali2010}
		\begin{align}
			(t_{I,0}^{(i)})^2
		...
		\begin{maxi!}
			{\boldsymbol{\boldsymbol{\Phi}}}{\tilde{z}(\boldsymbol{\Phi})}{\label{op:irs}}{\label{eq:su_irs_target}}
			\addConstraint{\sum_{n}{\log_2\left(1+\frac{(1-\rho)\mathrm{Tr}(\boldsymbol{C}_n\boldsymbol{\Phi})}{\sigma_n^2}\right)} \ge \bar{R}}\label{co:irs_rate}
		...
		\begin{algorithm}
			...
			\begin{algorithmic}[1]
				...
				\State \textbf{Output} $\theta_l^\star$
			\end{algorithmic}
		\end{algorithm}
	\end{subsection}
	...
	\begin{subsection}{Waveform and Splitting Ratio}
		\begin{subsubsection}{Geometric Programming}
			...
			\end{equation}
			Therefore, the original problem \ref{op:original} is reduced to an amplitude optimization problem
			\begin{maxi!}
	...
	\begin{subsection}{R-E Region Characterization}
		...
		\begin{subsubsection}{WPT}
			The current maximization problem can be solved by setting $\rho = 1$, removing the rate constraint in problem \ref{op:irs} and \ref{op:su_waveform} then solving them iteratively.
		\end{subsubsection}
		...
		\begin{subsubsection}{R-E Sample}
			...
			\begin{algorithm}
				...
				\begin{algorithmic}[1]
					...
					\Repeat
					\State $i \gets i + 1$
					\State Update IRS phase shift $\boldsymbol{\phi}^{(i)}$ by Algorithm \ref{al:irs}
