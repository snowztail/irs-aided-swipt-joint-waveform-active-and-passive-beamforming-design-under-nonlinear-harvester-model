...
\begin{section}{Single-User Optimization}
	...
	\begin{subsection}{IRS Phase Shift}
		...
		\begin{subsubsection}{Frequency-Flat IRS}
			...
			\end{equation}
			where $\boldsymbol{M}=[\boldsymbol{V}^H,\boldsymbol{h}_D]^H \in \mathbb{C}^{(L+1) \times N}$. On top of this, we formulate the FF-IRS phase shift optimization subproblem as follows. With given waveform $\boldsymbol{w}_{I/P}$ and splitting ratio $\rho$, introduce auxiliary variables
