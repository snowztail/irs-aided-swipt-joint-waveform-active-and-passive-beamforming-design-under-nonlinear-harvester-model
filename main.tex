...
\begin{document}
	...
	\begin{section}{Problem Formulation}\label{se:problem_formulation}
		...
		\begin{subsection}{Waveform and Splitting Ratio}
			...
			\end{maxi!}
			Following \cite{Clerckx2018b}, we introduce auxiliary variables $t'',\bar{\rho}$\footnote{It is trivial to conclude $\bar{\rho}^{\star}=1-\rho^{\star}$ because the R-E tradeoff is maximized when no signal is wasted at the receiver.} and transform problem~\eqref{op:waveform} into a reversed GP
			\begin{mini!}
