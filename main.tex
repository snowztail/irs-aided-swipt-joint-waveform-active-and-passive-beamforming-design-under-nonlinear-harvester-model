...
\begin{document}
	...
	\begin{section}{System Model}\label{se:system_model}
		...
		\begin{subsection}{Energy Harvester}
			...
			% Denote $i_{\text{S}}$ as the reverse bias saturation current, $n'$ as the diode ideality factor, $v_{\text{T}}$ as the thermal voltage, $v_{\text{D}}(t)=v_{\text{in}}(t)-v_{\text{out}}(t)$ as the voltage across the diode where $v_{\text{out}}(t)$ is the output voltage across the load. A Taylor expansion of the diode characteristic equation $i_{\text{D}}(t)=i_{\text{S}}(e^{v_{\text{D}}(t)/n' v_{\text{T}}}-1)$ around a quiescent operating point $a$ writes as $i_{\text{D}}(t)=\sum_{i=0}^{\infty}k_i'(v_{\text{D}}(t)-a)^i$, where $k_0'=i_{\text{S}}(e^{a/n' v_{\text{T}}}-1)$ and $k_i'=i_{\text{S}}e^{a/n'v_{\text{T}}}/i!(n'v_{\text{T}})^i$ for $i=1,\dots,\infty$. Note that this small-signal expansion model is only valid for the non-linear operation region, and the I-V relationship would be linear if the diode behavior is dominated by the load \cite{Clerckx2016a}. Also, an ideal low-pass filter with steady-state response can provide a constant $v_{\text{out}}$ that depends on the peak of $v_{\text{in}}(t)$ \cite{Curty2005}. Therefore, a proper choice of the operating voltage drop is $a=\mathcal{E}\{v_{\text{D}}(t)\}=-v_{\text{out}}$ such that
			% \begin{equation}\label{eq:i_D}
			% 	i_{\text{D}}(t)=\sum_{i=0}^{\infty}k_i'\rho^{i/2}R_{\text{ant}}^{i/2}y(t)^i\,.
			% \end{equation}
			...
			% 	i_{\text{out}}(t)=\mathcal{A}\{i_{\text{D}}(t)\}\approx\sum_{i=0}^{n_0}{k_i'}{\rho^{i/2}}{R_{\text{ant}}^{i/2}}\mathcal{E}\left\{{\mathcal{A}\left\{y(t)^i\right\}}\right\}\,.
			% \end{equation}
