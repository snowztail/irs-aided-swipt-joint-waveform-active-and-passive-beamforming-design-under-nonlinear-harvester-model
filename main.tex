...
\begin{document}
	...
	\begin{section}{System Model}\label{se:system_model}
		...
		\begin{subsection}{Transmit Signal}
			...
			\end{equation}
			where $w_{\text{I/P,}n,m}$ denotes the weight of the information/power signal transmitted by antenna $m$ at subband $n$. Stacking up the entries on all transmit antennas, we further define $\boldsymbol{w}_{\text{I/P,}n} \triangleq [w_{\text{I/P,}n,1},\dots,w_{\text{I/P,}n,M}]^T \in \mathbb{C}^{M \times 1}$ and $\boldsymbol{x}(t) \triangleq [x_1(t),\dots,x_M(t)]^T=\boldsymbol{x}_{\text{I}}(t)+\boldsymbol{x}_{\text{P}}(t) \in \mathbb{C}^{M \times 1}$, where
			\begin{align}
		...
		\begin{subsection}{Energy Harvester}
				...
				\begin{minipage}[b]{0.45\columnwidth}
					...
					\subfloat[A single diode rectifier\label{ci:single_diode_rectifier}]{
						\resizebox{\columnwidth}{!}{
							...
							\draw (0.25,2) to [D, v<=$v_{\text{D}}$, i=$i_{\text{D}}$] (2.25,2);
							\draw (2.25,2) to [short, -*] (2.5,2);
							\draw (2.5,2) to [C, l_=$C$, -*] (2.5,0);
							\draw (2.5,2) to [short] (4,2);
							\draw (4,2) to [R=$R_\text{L}$, v<=$v_{\text{out}}$, i=$i_{\text{out}}$] (4,0);
							\draw (4,0) to [short] (2,0) node[ground](GND){};
							\draw (2,0) to [short] (0,0);
						\end{circuitikz}
						}
			...
			\begin{figure*}[b]
				...
				\hrule
				\begin{flalign}
					\mathcal{E}\left\{\mathcal{A}\left\{y_{\text{I}}^2(t)\right\}\right\}
					& = \frac{1}{2}\sum_{n=1}^N{(\boldsymbol{h}_{n}^H\boldsymbol{w}_{\text{I,}n})(\boldsymbol{h}_{n}^H\boldsymbol{w}_{\text{I,}n})^H} = \frac{1}{2}\boldsymbol{h}^H\boldsymbol{W}_{\text{I,}0}\boldsymbol{h},&&\label{eq:y_I2}\\
					\mathcal{E}\left\{\mathcal{A}\left\{y_{\text{I}}^4(t)\right\}\right\}
					& = \frac{3}{4}\left(\sum_{n=1}^N{(\boldsymbol{h}_{n}^H\boldsymbol{w}_{\text{I,}n})(\boldsymbol{h}_{n}^H\boldsymbol{w}_{\text{I,}n})^H}\right)^2 = \frac{3}{4}(\boldsymbol{h}^H\boldsymbol{W}_{\text{I,}0}\boldsymbol{h})^2,&&\label{eq:y_I4}
				\end{flalign}
				\begin{flalign}
	...
	\begin{section}{Problem Formulation}\label{se:problem_formulation}
		...
		\begin{maxi!}
			{\boldsymbol{\phi},\boldsymbol{w}_{\text{I}},\boldsymbol{w}_\text{P},\rho}{z(\boldsymbol{\phi},\boldsymbol{w}_{\text{I}},\boldsymbol{w}_\text{P},\rho)}{\label{op:original}}{\label{ob:original}}
			\addConstraint{\frac{1}{2}\left(\lVert{\boldsymbol{w}_{\text{I}}}\rVert^2+\lVert{\boldsymbol{w}_\text{P}}\rVert^2\right)\le{P}}\label{co:original_power}
			\addConstraint{R(\boldsymbol{\phi},\boldsymbol{w}_{\text{I}},\rho) \ge \bar{R}}\label{co:original_rate}
		...
		\begin{subsection}{IRS Phase Shift}
			...
			\begin{algorithm}[!t]
				...
				\label{al:irs}
				\begin{algorithmic}[1]
					\State \textbf{input} $\beta_2,\beta_4,\boldsymbol{h}_{\text{D,}n},\boldsymbol{H}_{\text{I,}n},\boldsymbol{h}_{\text{R,}n},\boldsymbol{w}_{\text{I}},\boldsymbol{w}_\text{P},\rho,\sigma_n,\bar{R},Q,\epsilon$, $\forall n$
					...
					\Repeat
						\State $i \gets i + 1$
						\State Obtain $\boldsymbol{\Phi}^{(i)},t_{\text{I/P,}k}^{(i)}$, $\forall k$ by solving problem~\eqref{op:irs}
		...
		\begin{subsection}{Waveform, Active Beamforming and Splitting Ratio}
			...
			\begin{maxi!}
				{\boldsymbol{s}_{\text{I}},\boldsymbol{s}_\text{P},\rho}{z(\boldsymbol{s}_{\text{I}},\boldsymbol{s}_\text{P},\rho)}{\label{op:waveform}}{}
				\addConstraint{\frac{1}{2}\left(\lVert{\boldsymbol{s}_{\text{I}}}\rVert^2+\lVert{\boldsymbol{s}_\text{P}}\rVert^2\right)\le{P}}
				\addConstraint{R(\boldsymbol{s}_{\text{I}},\rho) \ge \bar{R}.}
			\end{maxi!}
			...
			\begin{mini!}
				{\boldsymbol{s}_{\text{I}},\boldsymbol{s}_\text{P},\rho,\bar{\rho},t'}{\frac{1}{t'}}{\label{op:waveform_rgp}}{}
				\addConstraint{\frac{1}{2}\left(\lVert{\boldsymbol{s}_{\text{I}}}\rVert^2+\lVert{\boldsymbol{s}_\text{P}}\rVert^2\right) \le P}\label{co:waveform_power}
				\addConstraint{\frac{t'}{z(\boldsymbol{s}_{\text{I}},\boldsymbol{s}_\text{P},\rho)} \le 1}\label{co:waveform_objective}
				\addConstraint{\frac{2^{\bar{R}}}{\prod_{n=1}^N \left(1+{\bar{\rho}\lVert{\boldsymbol{h}_n}\rVert^2 s_{\text{I,}n}^2}\big/{\sigma_n^2}\right)} \le 1}\label{co:waveform_rate}
			...
			\begin{align}
				z(\boldsymbol{s}_{\text{I}},\boldsymbol{s}_\text{P},\rho) &\triangleq \sum_{m_\text{P}}{g_{m_\text{P}}(\boldsymbol{s}_{\text{I}},\boldsymbol{s}_\text{P},\rho)}\label{eq:posynomial_objective},\\
				1+\frac{\bar{\rho}\lVert{\boldsymbol{h}_n}\rVert^2 s_{\text{I,}n}^2}{\sigma_n^2} &\triangleq \sum_{m_{\text{I,}n}}g_{m_{\text{I,}n}}(s_{\text{I,}n},\bar{\rho})\label{eq:posynomial_rate}
			\end{align}
			...
			\begin{mini!}
				{\boldsymbol{s}_{\text{I}},\boldsymbol{s}_\text{P},\rho,\bar{\rho},t'}{\frac{1}{t'}}{\label{op:waveform_gp}}{}
				\addConstraint{\frac{1}{2}\left(\lVert{\boldsymbol{s}_{\text{I}}}\rVert^2+\lVert{\boldsymbol{s}_\text{P}}\rVert^2\right)\le{P}}
				\addConstraint{{t'}\prod_{m_\text{P}}{\left(\frac{g_{{m_\text{P}}}(\boldsymbol{s}_{\text{I}},\boldsymbol{s}_\text{P},\rho)}{\gamma_{{m_\text{P}}}}\right)^{-\gamma_{{m_\text{P}}}}}\le{1}}
				\addConstraint{2^{\bar{R}}\prod_{n}\prod_{m_{\text{I,}n}}\left(\frac{g_{m_{\text{I,}n}}(s_{\text{I,}n},\bar{\rho})}{\gamma_{m_{\text{I,}n}}}\right)^{-\gamma_{m_{\text{I,}n}}}\le{1}}
				\addConstraint{\rho + \bar{\rho} \le 1}
			\end{mini!}
			...
			\end{align}
			$\boldsymbol{s}_{\text{I}},\boldsymbol{s}_\text{P},\rho$ are updated iteratively until convergence. Problem~\eqref{op:waveform_gp} can be solved using existing optimization tools such as CVX \cite{Grant2013}. The GP algorithm of the waveform, active beamforming and splitting ratio optimization is summarized in Algorithm~\ref{al:waveform}.
			\begin{algorithm}[!t]
				...
				\begin{algorithmic}[1]
					...
					\Repeat
						\State $i \gets i + 1$
						\State Update $\{\gamma_{m_\text{P}}^{(i)},\gamma_{m_{\text{I,}n}}^{(i)}\}$, $\forall n$ by \eqref{eq:gamma_P}, \eqref{eq:gamma_I}
		...
		\begin{subsection}{Alternating Optimization}
			For any direct, incident and reflective channels, we iteratively update the passive beamforming $\boldsymbol{\phi}$ by Algorithm~\ref{al:irs}, and the waveform, active beamforming $\boldsymbol{w}_{\text{I/P}}$ together with splitting ratio $\rho$ by Algorithm~\ref{al:waveform} until convergence. The AO algorithm is summarized in Algorithm~\ref{al:alternating}.
			\begin{algorithm}[!t]
				\caption{AO: Waveform, Active and Passive Beamforming, and Splitting Ratio.}
				\label{al:alternating}
				\begin{algorithmic}[1]
					\State \textbf{input} $\beta_2,\beta_4,\boldsymbol{h}_{\text{D,}n},\boldsymbol{H}_{\text{I,}n},\boldsymbol{h}_{\text{R,}n},P,\sigma_n,\bar{R},Q,\epsilon$, $\forall n$
					...
					\Repeat
						...
						\State Fix $\boldsymbol{\phi}^{(i)}$, update $\boldsymbol{h}_n^{(i)}$, $\forall n$ by \eqref{eq:h_n}, obtain $\boldsymbol{w}_{\text{I/P}}^{(i)}, \rho^{(i)}$ by Algorithm~\ref{al:waveform}
						\State Compute $z^{(i)}$ by \eqref{eq:z_irs}
					\Until $\lvert z^{(i)} - z^{(i-1)} \rvert \le \epsilon$
	...
	\begin{section}{Performance Evaluations}\label{se:performance_evaluation}
		\begin{figure}[!t]
			...
			\label{fi:layout}
		\end{figure}
		To evaluate the performance of the proposed IRS-aided SWIPT system, we characterize the average R-E regions under typical setups. Consider a large open space Wi-Fi-like environment at a center frequency of \SI{5.18}{\GHz} with bandwidth $B=1\,\si{\MHz}$ as reference. As shown in Fig.~\ref{fi:layout}, we assume the IRS moves along a horizontal path parallel to the AP-user path and let $d_{\text{H}}$, $d_{\text{V}}$ be the horizontal and vertical distances from the AP to the IRS, respectively. We also denote $d_{\text{D}}$, $d_{\text{I}}$, $d_{\text{R}}$ as the length of direct, incident, reflective paths such that $d_{\text{I}}=\sqrt{d_{\text{H}}^2+d_{\text{V}}^2}$, $d_{\text{R}}=\sqrt{(d_{\text{D}}-d_{\text{H}})^2+d_{\text{V}}^2}$. $d_{\text{D}}=15\,\si{\meter}$, $d_{\text{V}}=2\,\si{\meter}$ and $d_{\text{H}}=2\,\si{\meter}$ are chosen as reference. The path loss and fading parameters are obtained from IEEE TGn channel model D \cite{Erceg2004}, and the path loss is set to $L_0=-35\,\si{\dB}$ at $d_0=1\,\si{\meter}$. We assume all channels are NLoS with taps modelled as i.i.d. CSCG random variables of unit average sum-power, corresponding to a normalized multipath response. The reference numbers of transmit antennas, IRS elements and subbands are $M=1$, $L=20$, $N=16$, respectively. No spatial correlation is assumed across transmit antennas and IRS elements. Rectenna parameters are taken as $k_2=0.0034$, $k_4=0.3829$, $R_{\text{ant}}=50\,\si{\ohm}$ such that $\beta_2=0.17$ and $\beta_4=957.25$. With a \SI{0}{\dBi} transmit antenna gain, the average Effective Isotropic Radiated Power (EIRP) is fixed to $P=36\,\si{\dBm}$ and the average noise power is $\sigma_n=-40\,\si{\dBm}$ at all subbands as reference. We also assume a \SI{0}{\dBi} IRS element gain and a \SI{2}{\dBi} receive antenna gain. For the algorithm, the tolerance is set to $\epsilon=10^{-8}$, the number of candidates in the Gaussian randomization method is set to $Q=10^{3}$, and the R-E region is averaged over \num{300} channel realizations. In the R-E boundary, the leftmost point corresponds to WPT ($\rho=1$) where power can be allocated simultaneously to modulated and unmodulated waveform to maximize the average output DC current.
		...
		\begin{figure}[!t]
			...
			\end{adjustbox}
			\caption{Average R-E region versus $N$ for $M=1$, $L=20$, $\sigma_n=-40\,\si{\dBm}$, $B=1\,\si{\MHz}$ and $d_{\text{H}}=d_{\text{V}}=2\,\si{\meter}$.}
			\label{fi:re_subband}
		\end{figure}
		...
		\begin{figure}[!t]
			...
			\end{adjustbox}
			\caption{Average R-E region and splitting ratio versus $\sigma_n$ for $M=1$, $N=16$, $L=20$, $B=1\,\si{\MHz}$ and $d_{\text{H}}=d_{\text{V}}=2\,\si{\meter}$.}
			\label{fi:re_noise}
		\end{figure}
		...
		\begin{figure}[!t]
			...
			\end{adjustbox}
			\caption{Average R-E region versus $d_{\text{H}}$ for $M=1$, $N=16$, $L=20$, $\sigma_n=-40\,\si{\dBm}$, $B=1\,\si{\MHz}$ and $d_{\text{V}}=2\,\si{\meter}$.}
		...
		\begin{figure}[!t]
			...
			\end{adjustbox}
			\caption{Average R-E region versus $M$ for $N=16$, $L=20$, $\sigma_n=-40\,\si{\dBm}$, $B=1\,\si{\MHz}$ and $d_{\text{H}}=d_{\text{V}}=2\,\si{\meter}$.}
			\label{fi:re_tx}
		\end{figure}
		...
		\begin{figure}[!t]
			...
			\end{adjustbox}
			\caption{Average R-E region versus $L$ for $M=1$, $N=16$, $\sigma_n=-40\,\si{\dBm}$, $B=1\,\si{\MHz}$ and $d_{\text{H}}=d_{\text{V}}=2\,\si{\meter}$.}
		...
		\begin{figure}[!t]
			...
			\end{adjustbox}
			\caption{Average WIT SNR and WPT DC current versus $M$ for $L=20$, $N=16$, $\sigma_n=-40\,\si{\dBm}$, $B=1\,\si{\MHz}$, $d_{\text{H}}=0$ and $d_{\text{V}}=0.5\,\si{\meter}$.}
			\label{fi:scaling_tx}
		\end{figure}
		...
		\begin{figure}[!t]
			...
			\end{adjustbox}
			\caption{Average WIT SNR and WPT DC current versus $L$ for $M=1$, $N=16$, $\sigma_n=-40\,\si{\dBm}$, $B=1\,\si{\MHz}$, $d_{\text{H}}=0$ and $d_{\text{V}}=0.5\,\si{\meter}$.}
		...
		\begin{figure}[!t]
			...
			\end{adjustbox}
			\caption{Average R-E region for ideal, adaptive, fixed and no IRS over $B=1\,\si{\MHz}$ for $M=1$, $N=16$, $L=20$, $\sigma_n=-40\,\si{\dBm}$ and $d_{\text{H}}=d_{\text{V}}=2\,\si{\meter}$.}
		...
		\begin{figure}[!t]
			...
			\end{adjustbox}
			\caption{Average R-E region for ideal, adaptive, fixed and no IRS over $B=10\,\si{\MHz}$ for $M=1$, $N=16$, $L=20$, $\sigma_n=-40\,\si{\dBm}$ and $d_{\text{H}}=d_{\text{V}}=2\,\si{\meter}$.}
