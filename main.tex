...
\begin{section}{Problem Formulation}
	...
	\begin{maxi!}
		{\boldsymbol{w}_I,\boldsymbol{w}_P,\boldsymbol{\phi},\rho}{z(\boldsymbol{w}_I,\boldsymbol{w}_P,\boldsymbol{\phi},\rho)}{\label{op:original}}{\label{ob:original}}
		\addConstraint{\frac{1}{2}\left(\lVert{\boldsymbol{w}_I}\rVert^2+\lVert{\boldsymbol{w}_P}\rVert^2\right)\le{P}}
		\addConstraint{R(\boldsymbol{w}_I,\boldsymbol{\phi},\rho) \ge \bar{R}}\label{co:original_rate}
	...
	\begin{subsection}{IRS Phase Shift}
		...
		\begin{align}
			...
			& \quad - \frac{1}{4}(t_{I,0}^{(i-1)} + t_{P,0}^{(i-1)})^2 - \frac{1}{4}(t_{I,0}^{(i)} - t_{P,0}^{(i)})^2
		\end{align}
	...
	\begin{subsection}{Waveform and Splitting Ratio}
		...
		\end{align}
		$\boldsymbol{s}_I,\boldsymbol{s}_P,\rho$ are updated iteratively until convergence and the GP algorithm is summarized in Algorithm \ref{al:waveform}. It is also known as a inner approximation algorithm that only provides a KKT solution \cite{Marks1978a}.
		\begin{algorithm}
			...
			\begin{algorithmic}[1]
				...
				\Until $\lvert (z^{(i)} - z^{(i-1)}) / z^{(i)} \rvert \le \epsilon$
				\State Retrieve $\boldsymbol{w}_{I/P}^{\star}$ by \ref{eq:w_IP}
		...
		\begin{subsubsection}{R-E Sample}
			...
			\begin{algorithm}
				...
				\begin{algorithmic}[1]
					...
					\State Obtain splitting ratio $\rho^{(0)}$ and waveform $\boldsymbol{w}_{I/P}^{(0)}$ by GP (Algorithm \ref{al:waveform}) or inner loop (Algorithm \ref{al:inner_loop})
					\Repeat
					\State $i \gets i + 1$
					...
					\State Update splitting ratio $\rho^{(i)}$ and waveform $\boldsymbol{w}_{I/P}^{(i)}$ by GP (Algorithm \ref{al:waveform}) or inner loop (Algorithm \ref{al:inner_loop})
					\Until $\lvert (z^{(i)}-z^{(i-1)}) / z^{(i)} \rvert \le \epsilon$
