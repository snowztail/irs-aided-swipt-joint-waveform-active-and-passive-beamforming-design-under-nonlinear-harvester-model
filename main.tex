...
\begin{document}
	...
	\begin{section}{Problem Formulation}\label{se:problem_formulation}
		...
		\begin{subsection}{Low-Complexity Adaptive Design}
			The GP Algorithm~\ref{al:gp} achieves local optimality at a cost of exponential computational complexity w.r.t. the number of subbands \cite{Chiang2005}. To facilitate practical SWIPT implementation, we propose two low-complexity waveform amplitude design by combining WF and SMF under TS and PS setups. The optimal waveform design for WIT corresponds to the WF strategy that assigns the amplitude of modulated tone $n$ by
			...
			\end{equation}
			where the scaling ratio $\alpha \ge 1$ scales the matched filter to exploit the rectifier nonlinearity. $\alpha = 1$ corresponds to a standard matched filter where the amplitude of sinewave $n$ is linearly proportional to the equivalent channel strength $\lVert{\boldsymbol{h}_n}\rVert$, while $\alpha > 1$ further elevates the strong subbands and suppresses the weak ones.

			In the low-complexity TS waveform design, modulated waveform is used in the data session while multisine waveform is used in the energy session, and their amplitudes are given respectively by \eqref{eq:wf} and \eqref{eq:smf}. The R-E boundary is a segment between the WIT point ($\eta=0$) and the WPT point ($\eta=1$).

			% For the WIT point, we

			On the other hand, the low-complexity PS scheme controls the superposed waveform by balancing ratio $\delta$ and involves the design of splitting ratio $\rho$. The amplitude of modulated and multisine components are
			\begin{align}
			...
			Note that the balancing ratio $\delta$ denotes the proportion allocated to the multisine waveform while the splitting ratio $\rho$ denotes the proportion used for energy harvesting. For low-complexity PS design, we characterize the achievable R-E region by varying $\delta$ and $\rho$ from \num{0} to \num{1} such that the rate constraint \eqref{co:irs_rate} in the passive beamforming problem can be dropped (i.e. set $\bar{R}=0$).

			Moreover, the low-complexity TS design can be regarded as a special case where

			The low-complexity algorithm for joint waveform, active and passive beamforming design under TS and PS modes is summarized in Algorithm~.

			% In contrast, the R-E boundary of low-complexity TS design is a segment



			\begin{algorithm}[!t]
				\caption{Low-Complexity Design under TS and PS Modes.}
				\label{al:lc}
				\begin{algorithmic}[1]
					\State \textbf{Input} $\beta_2,\beta_4,\boldsymbol{h}_{\mathrm{D},n},\boldsymbol{V}_{n},P,\delta,\rho,\sigma_n,\bar{R}=0,Q,\epsilon$, $\forall n$
					\State \textbf{Initialize} $i \gets 0$, $\boldsymbol{\phi}^{(0)},\boldsymbol{b}_{\mathrm{I/P}}^{(0)},\boldsymbol{s}_{\mathrm{I/P}}^{(0)}$
					\State Get $\boldsymbol{w}_{\mathrm{I/P},n}^{(0)}$, $\forall n$ by \eqref{eq:w}
					\Repeat
						\State $i \gets i + 1$
						\State Get $\boldsymbol{\phi}^{(i)}$ based on $\boldsymbol{w}_{\mathrm{I/P}}^{(i-1)}$ by Algorithm~\ref{al:sca}
						\State Update $\boldsymbol{h}_n^{(i)}$, $\forall n$ by \eqref{eq:h_n}
						\State Get $\boldsymbol{s}_{\mathrm{I/P}}^{(i)}$ based on $\boldsymbol{h}_n^{(i)}$, $\forall n$ by \eqref{eq:wf}, {eq:smf}
						\State Update $\boldsymbol{w}_{\mathrm{I/P},n}^{(i)}$, $\forall n$ by \eqref{eq:w}
						\State Compute $z^{(i)}$ by \eqref{eq:z_waveform}
					\Until $\lvert z^{(i)} - z^{(i-1)} \rvert \le \epsilon$
					\State Set $\boldsymbol{\phi}^{\star}=\boldsymbol{\phi}^{(i)},\boldsymbol{w}_{\mathrm{I/P}}^{\star}=\boldsymbol{w}_{\mathrm{I/P}}^{(i)},\rho^{\star}=\rho^{(i)}$
