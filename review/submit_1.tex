...
\begin{document}
	...
	\begin{section}{System Model}\label{se:system_model}
		...
		\begin{subsection}{Transmitted Signal}
			...
			\begin{equation}
				\boldsymbol{x}_{\mathrm{I}}(t) = \Re{\left\{\sum_{n=1}^N\boldsymbol{w}_{\mathrm{I},n}\tilde{x}_{\mathrm{I},n}(t){e^{j2{\pi}{f_n}{t}}}\right\}},\quad \boldsymbol{x}_{\mathrm{P}}(t) = \Re{\left\{\sum_{n=1}^N\boldsymbol{w}_{\mathrm{P},n}{e^{j2{\pi}{f_n}{t}}}\right\}}
			\end{equation}
		...
		\begin{subsection}{Received Signal}
			...
			\begin{equation}
				y_{\mathrm{I}}(t) = \Re\left\{\sum_{n=1}^N{\boldsymbol{h}_{n}^H}{\boldsymbol{w}_{\mathrm{I},n}\tilde{x}_{\mathrm{I},n}(t)}{e^{j2{\pi}{f_n}{t}}}\right\}, \quad y_{\mathrm{P}}(t) = \Re\left\{\sum_{n=1}^N{\boldsymbol{h}_{n}^H}\boldsymbol{w}_{\mathrm{P},n}{e^{j2{\pi}{f_n}{t}}}\right\}
			\end{equation}
		...
		\begin{subsection}{Energy Harvester}
			...
			\begin{align}
				z(\boldsymbol{\phi},\boldsymbol{w}_{\mathrm{I}},\boldsymbol{w}_{\mathrm{P}},\rho)
				& = \beta_2\rho\Bigl(\mathbb{E}\left\{\mathbb{A}\left\{y_{\mathrm{I}}^2(t)\right\}\right\}+\mathbb{A}\left\{y_{\mathrm{P}}^2(t)\right\}\Bigr) +\beta_4\rho^2\Bigl(\mathbb{E}\left\{\mathbb{A}\left\{y_{\mathrm{I}}^4(t)\right\}\right\}+\mathbb{A}\left\{y_{\mathrm{P}}^4(t)\right\}&&\nonumber\\
	...
	\begin{section}{Problem Formulation}\label{se:problem_formulation}
		...
		\begin{subsection}{Passive Beamforming}
			...
			Plugging \eqref{eq:taylor_1}--\eqref{eq:coupled_terms} into \eqref{eq:z_irs}, we obtain the DC current approximation
			\begin{align}
		...
		\begin{subsection}{Waveform and Splitting Ratio}
			Next, we jointly optimize the waveform amplitude $\boldsymbol{s}_{\mathrm{I/P}}$ and the splitting ratio $\rho$ for any given IRS phase shift $\boldsymbol{\phi}$ and active precoder $\boldsymbol{b}_{\mathrm{I/P},n}$, $\forall n$. On top of \eqref{eq:b_n}, the equivalent channel strength at subband $n$ is $\lVert{\boldsymbol{h}_n}\rVert$, and \eqref{eq:R} and \eqref{eq:z_expand} reduce respectively to
			\begin{align}
				R(\boldsymbol{s}_{\mathrm{I}},\rho)
				& = \log_2\prod_{n=1}^N\left(1+\frac{(1-\rho)\lVert{\boldsymbol{h}_n}\rVert^2 s_{\mathrm{I},n}^2}{\sigma_n^2}\right),\label{eq:R_waveform}\\
			...
			\end{align}so that problem~\eqref{op:original} boils down to
	...
	\begin{appendix}
		\begin{subsection}{Proof of Proposition~\ref{pr:relaxation}}\label{ap:relaxation}
			...
			By applying rank constraint relaxation, problem~\eqref{ob:irs_equivalent}--\eqref{co:irs_equivalent_sd} is convex and satisfies the Slater's condition \cite{Boyd2004}, thus strong duality holds. The corresponding Lagrangian at iteration $i$ is given by
			\begin{align}\label{eq:lagrangian}
				...
				&= \mathrm{tr}\left(\boldsymbol{\Phi}^{(i)}\right) - \frac{1}{2} \beta_2 \rho \mathrm{tr}\Bigl(
					...
					\Bigr) - \frac{3}{8} \beta_4 \rho^2 \Biggl(
						...
						\Bigr)
						\nonumber\\
				&\quad+ \sum_{k=-N+1}^{N-1} 2 (t_{\mathrm{P},k}^{(i-1)})^* \mathrm{tr}\Bigl(
					\boldsymbol{C}_{\mathrm{P},k} \boldsymbol{\Phi}^{(i)}
					\Bigr) + 2 (t_{\mathrm{I},0}^{(i-1)} + t_{\mathrm{P},0}^{(i-1)}) \mathrm{tr}\Bigl(
							(\boldsymbol{C}_{\mathrm{I},0} + \boldsymbol{C}_{\mathrm{P},0}) \boldsymbol{\Phi}^{(i)}
						\Bigr)\nonumber\\
				&\quad- \mathrm{tr}^2 \Bigl(
					(\boldsymbol{C}_{\mathrm{I},0} - \boldsymbol{C}_{\mathrm{P},0}) \boldsymbol{\Phi}^{(i)}
					\Bigr)
					\Biggr) + \mu \Biggl(
			...
			\end{equation}
			where $\boldsymbol{\Delta}^\star$ is given by
			\begin{align}\label{eq:delta}
				\boldsymbol{\Delta}^\star
				&= \frac{1}{2} \beta_2 \rho (\boldsymbol{C}_{\mathrm{I},0}+\boldsymbol{C}_{\mathrm{P},0}) + \frac{3}{4} \beta_4 \rho^2
					\Biggl(
						2 t_{\mathrm{I},0}^{(i-1)} \boldsymbol{C}_{\mathrm{I},0} + \sum_{k=-N+1}^{N-1} (t_{\mathrm{P},k}^{(i-1)})^* \boldsymbol{C}_{\mathrm{P},k}\nonumber\\
				&\quad+ (t_{\mathrm{I},0}^{(i-1)} + t_{\mathrm{P},0}^{(i-1)}) (\boldsymbol{C}_{\mathrm{I},0} + \boldsymbol{C}_{\mathrm{P},0}) - (\boldsymbol{C}_{\mathrm{I},0} - \boldsymbol{C}_{\mathrm{P},0}) \mathrm{tr}\Bigl(
