...
\begin{document}
	...
	\begin{reviewer}
		...
		\begin{response}
			...
			% As for the measurements, we evaluate the converging speed of all proposed algorithms. Figs.~\ref{fi:convergence_sca}--\ref{fi:convergence_bcd} show the output DC current versus the number of iterations for one channel realization at the WPT point. It is observed that the SCA- and BCD-based algorithms typically converge within \num{3} iterations while the GP-based algorithm takes around \num{9} iterations to converge. Those figures are not added to the manuscript due to the space constraint.
			% \begin{figure}[!t]
			% 	\centering
			% 	\subfloat[SCA-based\label{fi:convergence_sca}]{
			% 		\resizebox{0.32\columnwidth}{!}{
			% 			\input{../assets/convergence_sca.tex}
			% 		}
			% 	}
			% 	\subfloat[GP-based\label{fi:convergence_gp}]{
			% 		\resizebox{0.32\columnwidth}{!}{
			% 			\input{../assets/convergence_gp.tex}
			% 		}
			% 	}
			% 	\subfloat[BCD-based\label{fi:convergence_bcd}]{
			% 		\resizebox{0.32\columnwidth}{!}{
			% 			\input{../assets/convergence_bcd.tex}
			% 		}
			% 	}
			% 	\caption{Converging trend of the proposed algorithms for $M=1$, $N=16$, $L=20$, $\sigma_n^2=\SI{-40}{\dBm}$, $B=\SI{1}{\MHz}$ and $d_{\mathrm{H}}=d_{\mathrm{V}}=\SI{2}{\meter}$..}
			% \end{figure}
			\label{re:1.1}
		\end{response}
	...
	\begin{reviewer}
		...
		\begin{response}
			...
			\begin{figure}[!h]
				...
				\resizebox{0.32\columnwidth}{!}{
						\input{../assets/waveform_wit_modulated_100reflectors.tex}
					}
				}
				\subfloat[WPT waveform: modulated]{
					\resizebox{0.32\columnwidth}{!}{
						\input{../assets/waveform_wpt_modulated_100reflectors.tex}
					}
				}
				\subfloat[WPT waveform: multisine]{
					\resizebox{0.32\columnwidth}{!}{
						\input{../assets/waveform_wpt_multisine_100reflectors.tex}
					}
				}
		...
		\begin{response}
			...
			\begin{framed}
				...
				\textellipsis the LC-BCD algorithm draws the R-E tradeoff by performing a two-dimensional search that adjusts $\delta$ and $\rho$ from \num{1} to \num{0}.
	...
	\begin{reviewer}
		...
		\begin{response}
			Thank you for the clarification. The discrete phase shift is obtained by quantizing the continuous phase shift obtained by Algorithm~\ref{M-al:bcd} to the uniform IRS codebook $\mathcal{C}_\phi = \{e^{j 2 \pi i / 2^b} \mid i = 1, \dots, 2^b\}$. We are aware that quantization can lead to performance loss compared with direct optimization over feasible phase shift set. In the revised manuscript, we have emphasized this point as follows.
			\begin{framed}
		...
		\begin{response}
			We have incorporated aforementioned references into the manuscript as follows.
			\begin{framed}
