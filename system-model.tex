...
\begin{section}{Problem Formulation}
	...
	\begin{subsection}{Single-User Optimization}
		...
		\end{maxi}
		\ref{op:1} is nontrivial due to the non-convex objective function, unit-modulus constraint, and coupled variables. To reduce the design complexity, we propose an suboptimal alternating optimization algorithm that updates the phase shifts at the IRS and the information weight at the transmitter iteratively until convergence.

		The phase optimization problem can be formed as follows. For a fixed information weight $\boldsymbol{w}_I$ and power-splitting ratio $\rho$, problem \ref{op:1} reduces to
		\begin{maxi}
			{\boldsymbol{\boldsymbol{v}}}{\sum_{n}{\log_2\left(1+\frac{(1-\rho)\lvert(h_{D,n}+\boldsymbol{v}^H{\boldsymbol{\Phi}_n})\rvert^2\lvert{w}_{I,n}\rvert^2}{\sigma_n^2}\right)}}{}{}
		\end{maxi}
		...
		% Define $\boldsymbol{v}=[e^{j{\theta_1}},\dots,e^{j{\theta_L}}]^H \in \mathbb{C}^{L \times 1}$ and $\Phi=\mathrm{diag}\left\{\boldsymbol{h}_{R,n}\right\}\boldsymbol{h}_{I,n} \in \mathbb{C}^{L \times 1}$. On top of this, \ref{eq:h_{k,n}} can be expressed as
		% \begin{equation}
		% 	\boldsymbol{h}_n=\boldsymbol{h}_{D,n}+\boldsymbol{v}^H{\Phi}
		% \end{equation}
