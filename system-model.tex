	...
	\begin{subsection}{Frequency-Flat IRS}
		...
		\begin{subsubsection}{Receive Splitting Ratio Optimization}
			...
			\begin{maxi!}
				...
				\addConstraint{0 \le \rho \le 1}
			\end{maxi!}
			It can be observed that $z(\rho)$ is a monotonically increasing function of $\rho$ such that the maximum value of \ref{eq:su_ratio_target} is achieved when equality in \ref{eq:su_ratio_constraint} holds, namely
			\begin{equation}\label{eq:rho}
				\prod_n \left(1 + \frac{(1 - \rho^\star)\lvert h_n w_n \rvert^2}{\sigma_n^2}\right) = 2^{\bar{R}}
			\end{equation}
			The optimal power splitting ratio can be obtained from \ref{eq:rho} in closed form.
		\end{subsubsection}
