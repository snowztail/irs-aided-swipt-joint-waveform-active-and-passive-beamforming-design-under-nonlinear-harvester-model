...
\begin{section}{Problem Formulation}
	...
	\begin{subsection}{Single-user optimization}
		In this section, we first characterize the rate-energy region in the single-user setup. Define $w_{I,n}=s_{I,n}e^{j{\phi}_n}$ and $w_{P,n}=s_{P,n}e^{j{\phi}_n}$, it can be observed from \ref{eq:R_k} and \ref{eq:z_k_standard} that the optimal information and power phase are matched to frequency response as
		\begin{equation}\label{eq:phi_n}
			\phi_{I,n}^\star=\phi_{P,n}^\star=-\angle{h_n}
		\end{equation}
		\ref{eq:phi_n} maximizes rate and energy simultaneously and depends on the IRS design.
	\end{subsection}
\end{section}
