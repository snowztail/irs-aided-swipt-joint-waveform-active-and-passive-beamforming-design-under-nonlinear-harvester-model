...
\begin{section}{Problem Formulation}
	...
	\begin{subsection}{Single-User Optimization}
		In this section, we first characterize the rate-energy region in the single-user setup. Define $h_n={A_n}{e^{j{\bar{\psi}_n}}}$, $w_{I,n}=s_{I,n}e^{j{\phi}_n}$, $w_{P,n}=s_{P,n}e^{j{\phi}_n}$
		, $\boldsymbol{v}=[e^{j{\theta_1}},\dots,e^{j{\theta_L}}]^H \in \mathbb{C}^{L \times 1}$, and $\boldsymbol{\Phi}_n=\mathrm{diag}\left\{\boldsymbol{h}_{R,n}\right\}\boldsymbol{h}_{I,n} \in \mathbb{C}^{L \times 1}$. On top of this, the composite channel \ref{eq:h_{k,n}} can be expressed as $h_n=h_{D,n}+\boldsymbol{v}^H{\boldsymbol{\Phi}_n}$. The WSR-E region can be obtained through a rate maximization problem subject to transmit power and output DC current constraints
		\begin{maxi}
			...
			{\boldsymbol{w}_I,\boldsymbol{w}_P,\boldsymbol{v},\rho}{\sum_{n}{\log_2\left(1+\frac{(1-\rho)\lvert(h_{D,n}+\boldsymbol{v}^H{\boldsymbol{\Phi}_n})w_{I,n}\rvert^2}{\sigma_n^2}\right)}}{\label{op:1}}{}
		...
		\end{maxi}
		\ref{op:1} is intricate due to the non-convex objective function, unit-modulus constraints, and coupled variables. To reduce the design complexity, we propose an suboptimal alternating optimization algorithm that updates the phase shifts at the IRS and the information weight at the transmitter iteratively until convergence.
		...
		\begin{maxi}
			{\boldsymbol{v}}{\sum_{n}{\log_2\left(1+\frac{(1-\rho)\lvert{w}_{I,n}\rvert^2\lvert(h_{D,n}+\boldsymbol{v}^H{\boldsymbol{\Phi}_n})\rvert^2}{\sigma_n^2}\right)}}{\label{op:temp}}{}
			% \addConstraint{z(\boldsymbol{w}_I,\boldsymbol{w}_P,\boldsymbol{v},\rho)\ge{\bar{z}}}
			\addConstraint{\lvert{v_l}\rvert=1,\quad l=1,\dots,L}
		\end{maxi}
		Note that $\lvert(h_{D,n}+\boldsymbol{v}^H{\boldsymbol{\Phi}_n})\rvert^2=\lvert{h}_{D,n}\rvert^2+h_{D,n}\boldsymbol{\Phi}_n^H\boldsymbol{v}+\boldsymbol{v}^H\boldsymbol{\Phi}_n{h_{D,n}^*}+\boldsymbol{v}^H\boldsymbol{\Phi}\boldsymbol{\Phi}^H\boldsymbol{v}$. Introduce an auxiliary variable $t$ with unit modulus and rewrite \ref{op:temp} as
		\begin{maxi}
			{\bar{\boldsymbol{v}}}{\sum_{n}{\log_2\left(1+\frac{(1-\rho)\lvert{w}_{I,n}\rvert^2(\lvert{h_{D,n}}\rvert^2+\bar{\boldsymbol{v}}^H\boldsymbol{R}\bar{\boldsymbol{v}})}{\sigma_n^2}\right)}}{\label{op:temp1}}{}
		...
		\begin{equation}
			\boldsymbol{R}=
			\begin{bmatrix}
				\boldsymbol{\Phi}_n\boldsymbol{\Phi}_n^H & \boldsymbol{\Phi}_n{h}_{D,n}^* \\
				h_{D,n}\boldsymbol{\Phi}_n               & 0
			\end{bmatrix},
			...
			\end{bmatrix}
		\end{equation}
		However, problem \ref{op:temp1} is still non-convex. We further define a positive semidefinite matrix $\boldsymbol{V}=\bar{\boldsymbol{v}}\bar{\boldsymbol{v}}^H$ with $\mathrm{rank}(\boldsymbol{V})=1$ such that $\bar{\boldsymbol{v}}\boldsymbol{R}\bar{\boldsymbol{v}}=\mathrm{Tr}(\boldsymbol{R}\bar{\boldsymbol{v}}\bar{\boldsymbol{v}}^H)=\mathrm{Tr}(\boldsymbol{R}\boldsymbol{V})$.
		A semidefinite programming (SDP) problem is then formulated by applying SDR to drop the rank-one constraint as
		\begin{maxi}
			{\boldsymbol{V}}{\sum_{n}{\log_2\left(1+\frac{(1-\rho)\lvert{w}_{I,n}\rvert^2(\lvert{h_{D,n}}\rvert^2+\mathrm{Tr}(\boldsymbol{R}\boldsymbol{V}))}{\sigma_n^2}\right)}}{\label{op:temp3}}{}
			\addConstraint{\boldsymbol{V}_{l,l}=1,l=1,\dots,L+1}
			\addConstraint{\boldsymbol{V}\succeq{0}}
		\end{maxi}
		which can be solved by softwares as CVX \cite{Grant2013}.
		...
		Once a feasible phase shift set is obtained, the original problem \ref{op:1} can be reduced to
		\begin{maxi}
