	...
	\begin{subsection}{Frequency-Flat IRS}
		...
		\begin{subsubsection}{IRS Phase Shift Optimization}\label{se:fs_irs}
			...
			\begin{algorithm}
				...
				\label{al:ff_irs}
				\begin{algorithmic}[1]
					\State \textbf{Input} $\beta_2,\beta_4,Q,\bar{R},\epsilon,\rho,\boldsymbol{w}_{I/P},\boldsymbol{M},\sigma_n,\boldsymbol{R}_n,\boldsymbol{C}_{I/P,n} \ \forall n$ by \ref{eq:R_n,phi}, \ref{eq:C_{I/P,n}}
		...
		\begin{subsubsection}{Transmit Waveform Optimization}
			...
			\begin{algorithm}
				...
				\begin{algorithmic}[1]
					\State \textbf{Input} $\beta_2,\beta_4,P,Q,\bar{R},\epsilon,\rho,\sigma_n,\boldsymbol{H}_n,\boldsymbol{R}_n \ \forall n$ by \ref{eq:R_n,phi}
		...
		\begin{subsubsection}{WIT Initialization}
			...
			\begin{algorithm}
				...
				\label{al:ff_wit}
				\begin{algorithmic}[1]
					\State \textbf{Input} $h_D, h_I, h_R, P, Q, \sigma_n, \boldsymbol{R}_n \ \forall n$ by \ref{eq:R_n,phi}
					\State \textbf{Initialize} $i \leftarrow 0$, $\boldsymbol{\phi}^{(0)},\boldsymbol{w}_I^{(0)}$
					\Repeat
					\State $i \leftarrow i + 1$
