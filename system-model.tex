...
\begin{section}{System Model}
	...
	\begin{subsection}{Transmitted Signal}
		\begin{subsubsection} {Modulated Information Waveform}
			...
			\end{equation}
			where $w_{I,k,n}$ is the corresponding information weight. Let $\boldsymbol{W}_I=[\boldsymbol{w}_{I,1},\dots,\boldsymbol{w}_{I,N}] \in \mathbb{C}^{K \times N}$ with $\boldsymbol{w}_{I,n}=[w_{I,1,n},\dots,w_{I,K,n}]^T \in \mathbb{C}^{K \times 1}$. On top of this, the RF transmit information waveform at time $t$ writes as
		...
		\begin{subsubsection}{Unmodulated Power Waveform}
			...
			\end{equation}
			where $w_{P,k,n}$ is the corresponding power weight. Let $\boldsymbol{W}_P=[\boldsymbol{w}_{P,1},\dots,\boldsymbol{w}_{P,N}] \in \mathbb{C}^{K \times N}$ with $\boldsymbol{w}_{P,n}=[w_{P,1,n},\dots,w_{P,K,n}]^T \in \mathbb{C}^{K \times 1}$. Then, the transmit power signal at time $t$ is given by
	...
	\begin{subsection}{Received Signal}
		...
		\end{equation}
		with the information and power components equal
	...
	\begin{subsection}{Information Decoder}
		One major benefit of using proposed waveform is that the deterministic power term $y_{P,k}(t)$ bears no information and creates no interference to the modulated information term $y_{I,k}(t)$. Besides, each user treats the information signals from other users as interference such that the SINR at subband $n$ of user $k$ is
		\begin{equation}\label{eq:gamma_{k,n}}
			\gamma_{k,n}=\frac{(1-\rho)(w_{I,k,n}h_{k,n})(w_{I,k,n}h_{k,n})^*}{\sum_{j=1,j{\ne}k}^N{(1-\rho)(w_{I,j,n}h_{I,k,n})(w_{I,j,n}h_{I,k,n})^*+\sigma_n^2}}
		\end{equation}
		...
		\end{equation}
		A significant conclusion in \cite{Clerckx2018b} indicates that the rate \ref{eq:R_k} is always achievable with or without waveform cancellation, by subtracting the deterministic power component or constructing a translated codebook. Hence, the weighted sum rate is
		\begin{equation}\label{eq:R}
			R=\sum_{k=1}^K{u_kR_k}
		\end{equation}
		where $u_k$ is the weight of user $k$.
	\end{subsection}
...
\begin{section}{Problem Formulation}
	In this section, we first define the weighted sum rate-energy (WSR-E) region then formulate specific waveform and IRS optimization problems.
	\begin{subsection}{Weighted Sum Rate-Energy Region}
		We define the achievable WSR-E region as
		\begin{equation}
			C_{R-I}(P) \triangleq \left\{(R,I):\right\}
		\end{equation}
	\end{subsection}
\end{section}
