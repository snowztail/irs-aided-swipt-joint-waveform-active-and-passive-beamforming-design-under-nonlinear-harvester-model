...
\begin{section}{System Model}
	...
	\begin{subsection}{Composite Channel Model}
		...
		\end{equation}
		represents the overall channel gain of user $k$ at subband $n$. We further define $\boldsymbol{h}_{k,n}=[\underbrace{h_{k,n},\dots,h_{k,n}}_{N\text{ times}}]^T \in \mathbb{C}^{K \times 1}$. On top of this, the $n$-th subchannel for all users $\boldsymbol{h}_n \in \mathbb{C}^{K \times 1}$ can be expressed as
	...
	\begin{subsection}{Energy Harvester}
		...
		\end{equation}
		where $k_i=i_s/i!(nv_t)^i$. We take $n_0=4$ to investigate the fundamental impact of diode nonlinearity on power transfer. Note that $\mathcal{E}\{\mathcal{A}\{y_{I,k}(t)y_{P,k}(t)\}\}=\mathcal{E}\{\mathcal{A}\{y_{I,k}^3(t)y_{P,k}(t)\}\}=\mathcal{E}\{\mathcal{A}\{y_{I,k}(t)y_{P,k}^3(t)\}\}=0$, $\mathcal{E}\{\mathcal{A}\{y_{I,k}^2(t)y_{P,k}^2(t)\}\}=\mathcal{A}\{\mathcal{E}\{y_{I,k}^2(t)\}\}\mathcal{A}\{y_{P,k}^2(t)\}$, and the remaining terms are expressed by \ref{eq:E{A{y_{I,k}**2}}} -- \ref{eq:A{y_{P,k}**4'}}. Since $\mathcal{E}\left\{\lvert\tilde{x}_{k,n}\rvert^2\right\}=1$ and $\mathcal{E}\left\{\lvert\tilde{x}_{k,n}\rvert^4\right\}=2$, modulation provides a power gain to the nonlinear terms in the output DC current.
		\begin{figure*}[b]
			\hrule
			\begin{align}
				...
				\mathcal{A}\left\{y_{P,k}^4(t)\right\}
				 ...
				 & =\frac{3}{8}\sum_{\substack{{n_1},{n_2},{n_3},{n_4}\\{n_1}+{n_2}={n_3}+{n_4}}}{({\boldsymbol{w}_{P,n_3}^H}{\boldsymbol{h}_{k,n_3}^*}{\boldsymbol{h}_{k,n_1}^T}{\boldsymbol{w}_{P,n_1}})({\boldsymbol{w}_{P,n_4}^H}{\boldsymbol{h}_{k,n_4}^*}{\boldsymbol{h}_{k,n_2}^T}{\boldsymbol{w}_{P,n_2}})}\label{eq:A{y_{P,k}**4'}}
...
\begin{section}{Problem Formulation}
	...
	\begin{subsection}{Weighted Sum Rate-Energy Region}
		...
		\begin{equation}
			C_{R-I}(P) \triangleq \left\{(R,I):R\le\sum_{k=1}^K{u_{I,k}R_k},I\le\sum_{k=1}^K{u_{P,k}i_k,?}\right\}
		\end{equation}
		where $u_{I,k},u_{P,k}$ denote the information and power priority of user $k$.
	\end{subsection}
\end{section}
