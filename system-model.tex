...
\begin{section}{Problem Formulation}
	...
	\begin{subsection}{Single-User Optimization}
		...
		\begin{subsubsection}{Frequency-Flat IRS}
			...
			\begin{maxi}
				{\boldsymbol{V}}{\sum_{n}{\log_2\left(1+\frac{(1-\rho)\lvert{w}_{I,n}\rvert^2\mathrm{Tr}(\boldsymbol{R}_n\boldsymbol{V})}{\sigma_n^2}\right)}}{\label{op:temp3}}{}
				\addConstraint{z(\boldsymbol{V})\ge{\bar{z}}}
				\addConstraint{\boldsymbol{V}_{l,l}=1,l=1,\dots,L+1}
				\addConstraint{\boldsymbol{V}\succeq{0}}
				\addConstraint{\mathrm{rank}(\boldsymbol{V})=1}
			\end{maxi}
			To tackle the non-convex current constraint, we define waveform matrices $\boldsymbol{M}_{I/P}=\boldsymbol{w}_{I/P}^*\boldsymbol{w}_{I/P}^T$. On top of this, let $\boldsymbol{M}_{I/P, n}$ keep the $n$-th ($n=-N+1,\dots,N-1$) diagonal of $\boldsymbol{M}_{I/P}$ and nulls the remaining entries. Therefore, the current expression \ref{eq:z_k_truncated} is transformed to \ref{eq:z_k_irs} with $\boldsymbol{h}=\boldsymbol{h}_D+\boldsymbol{\Phi}^H\boldsymbol{v} \in \mathbb{C}^{N \times 1}$ and $\boldsymbol{\Phi}^H=[\boldsymbol{\Phi}_1,\dots,\boldsymbol{\Phi}_N]^H \in \mathbb{C}^{N \times L}$.
			\begin{figure*}[b]
				\hrule
			...
			\begin{equation}\label{eq:t}
				\begin{split}
					t_{I/P,n}
					&=\boldsymbol{h}^H\boldsymbol{M}_{I/P,n}\boldsymbol{h}=\mathrm{Tr}(\boldsymbol{h}\boldsymbol{h}^H\boldsymbol{M}_{I/P,n})\\
					&=\mathrm{Tr}(\boldsymbol{R}^H\boldsymbol{V}\boldsymbol{R}\boldsymbol{M}_{I/P,n})
				\end{split}
			\end{equation}
			where $\boldsymbol{R}^H=[\boldsymbol{\Phi}^H,\boldsymbol{h}_D] \in \mathbb{C}^{N \times (L+1)}$. It holds that $t_{I/P,n}t_{I/P,n}^*=t_{I/P,-n}t_{I/P,-n}^*$ because $\boldsymbol{M}_{I/P,-n}=\boldsymbol{M}_{I/P,n}^H$. On top of this, define $\boldsymbol{t}_{I/P}=[t_{I/P,1},\dots,t_{I/P,N-1}]^T \in \mathbb{C}^{(N-1) \times 1}$ and rewrite \ref{eq:z_k_irs} as
