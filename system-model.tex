...
\begin{section}{Single-User Optimization}
	...
	\begin{subsection}{Frequency-Flat IRS}
		...
		\begin{equation}
			\boldsymbol{h}\boldsymbol{h}^H=\boldsymbol{R}^H\boldsymbol{\Phi}\boldsymbol{R}
		\end{equation}
		where $\boldsymbol{R}=[\boldsymbol{V}^H,\boldsymbol{h}_D]^H \in \mathbb{C}^{(L+1) \times N}$. Hence, the output current \ref{eq:z_k_channel} can be reformulated as $z(\boldsymbol{\Phi})$ (details given in section \ref{se:fs_irs}). To reduce the design complexity, we propose an suboptimal alternating optimization algorithm that iteratively updates the phase shifts and the waveforms with the other being fixed.

		\begin{subsubsection}{Phase shift optimization}\label{se:fs_irs}
			The phase optimization subproblem is formed as follows. With a given waveform $\boldsymbol{w}_I,\boldsymbol{w}_P,\rho$, problem \ref{op:su} reduces to
			\begin{maxi!}
				{\boldsymbol{\boldsymbol{\Phi}}}{z(\boldsymbol{\Phi})}{\label{op:su_irs}}{\label{eq:su_irs_target}}
			...
			\begin{equation}\label{eq:t}
				\begin{split}
					...
					&=\mathrm{Tr}(\boldsymbol{R}^H\boldsymbol{\Phi}\boldsymbol{R}\boldsymbol{W}_{I/P,n}^*)\\
					&=\mathrm{Tr}(\boldsymbol{R}\boldsymbol{W}_{I/P,n}^*\boldsymbol{R}^H\boldsymbol{\Phi})\\
					&=\mathrm{Tr}(\boldsymbol{C}_{I/P,n}\boldsymbol{\Phi})
				\end{split}
			\end{equation}
			where we define $\boldsymbol{C}_{I/P,n}=\boldsymbol{R}\boldsymbol{W}_{I/P,n}^*\boldsymbol{R}^H \in \mathbb{C}^{(L+1)\times(L+1)}$. Therefore, \ref{eq:z_k_channel} rewrites as
			...
			\begin{equation}
				\begin{split}
					...
					& \quad + \mathrm{Tr}(t_{I/P,n}^{(i-1)}\boldsymbol{C}_{I/P,n}^H\boldsymbol{\Phi}^{(i)})\\
					& \quad- t_{I/P,n}^{(i-1)} (t_{I/P,n}^{(i-1)})^*
				\end{split}
			\end{equation}
			and
			...
			\begin{equation}\label{eq:z_irs_approx}
				\begin{split}
					...
					& - \frac{3}{8} \beta_4 \rho^2 \sum_{n=-N+1}^{N-1} 2t_{I,n}^{(i-1)} (t_{I,n}^{(i-1)})^* + t_{P,n}^{(i-1)} (t_{P,n}^{(i-1)})^* \\
					& - \frac{3}{2} \beta_4 \rho^2 t_{I,0}^{(i-1)} t_{P,0}^{(i-1)}
				\end{split}
			\end{equation}
			...
			\begin{equation}\label{eq:A}
				...
				\end{split}
			\end{equation}
			We then plug \ref{eq:z_irs_approx} into problem \ref{op:su_irs}, relax the rank constraint \ref{co:irs_rank}, and obtain the optimal IRS matrix $\boldsymbol{\Phi}^{\star}$ by interior-point method. If $\mathrm{rank}(\boldsymbol{\Phi}^{\star})=1$, the optimal phase shift vector $\bar{\boldsymbol{\phi}}^\star$ is attained by eigenvalue decomposition (EVD). Otherwise, a best feasible candidate $\bar{\boldsymbol{\phi}}^\star$ can be extracted through Gaussian randomization method \cite{Huang2010}. First, perform EVD on $\boldsymbol{\Phi}^{\star}$ as $\boldsymbol{\Phi}^{\star}=\boldsymbol{U}_{\boldsymbol{\Phi}^{\star}}\boldsymbol{\Sigma}_{\boldsymbol{\Phi}^{\star}}\boldsymbol{U}_{\boldsymbol{\Phi}^{\star}}^H$. Then, generate $R$ CSCG random vectors $\boldsymbol{r}_r \sim \mathcal{CN}(\boldsymbol{0},\boldsymbol{I}_{L+1}),\ r=1,\dots,R$ and construct the corresponding candidates $\bar{\boldsymbol{\phi}}_r=\boldsymbol{U}_{\boldsymbol{\Phi}^{\star}}\boldsymbol{\Sigma}_{\boldsymbol{\Phi}^{\star}}^{\frac{1}{2}}\boldsymbol{r}_r$. Next, the optimal solution $\bar{\boldsymbol{\phi}}^\star$ is approximated by the one achieving maximum objective value \ref{eq:su_irs_target}. Finally, we can retrieve the phase shift by $\theta_l=\arg(\phi_l^\star/\phi_{L+1}^\star)$. The algorithm for the phase optimization subproblem is summarized in Algorithm \ref{al:ff_irs}.
			\begin{algorithm}
				...
				\begin{algorithmic}[1]
					...
					\State Construct candidate IRS vectors $\bar{\boldsymbol{\phi}}_r=\boldsymbol{U}_{\boldsymbol{\Phi}^{\star}}\boldsymbol{\Sigma}_{\boldsymbol{\Phi}^{\star}}^{\frac{1}{2}}\boldsymbol{r}_r$ and corresponding matrices $\boldsymbol{\Phi}_r=\bar{\boldsymbol{\phi}}_r\bar{\boldsymbol{\phi}}_r^H  \ \forall r$
		...
		\begin{subsubsection}{Waveform optimization}
			The waveform optimization subproblem is formed as follows. Once $\boldsymbol{\Phi}$ is obtained, problem \ref{op:su} is reduced to
			\begin{maxi}
				{\boldsymbol{W}_I,\boldsymbol{W}_P,\rho}{z(\boldsymbol{W}_I,\boldsymbol{W}_P,\rho)}{\label{op:su_waveform}}{}
				\addConstraint{\sum_{n}{\log_2\left(1+\frac{(1-\rho){W}_{I,n,n}\mathrm{Tr}(\boldsymbol{R}_n\boldsymbol{\Phi})}{\sigma_n^2}\right)} \ge \bar{R}}
				\addConstraint{\frac{1}{2}(\mathrm{Tr}(\boldsymbol{W}_I)+\mathrm{Tr}(\boldsymbol{W}_P))\le{P}}
			...
			\begin{equation}
				\begin{split}
					...
					& = 2 \Re \left\{\mathrm{Tr}((t_{I/P,n}^{\prime (k-1)})^*\boldsymbol{M}_{n}\boldsymbol{W}_{I/P}^{(i)})\right\} - t_{I/P,n}^{\prime (k-1)} (t_{I/P,n}^{\prime (k-1)})^*\\
					& = \mathrm{Tr}\left((t_{I/P,n}^{\prime (k-1)})^*\boldsymbol{M}_{n}\boldsymbol{W}_{I/P}^{(i)}\right) + \mathrm{Tr}(t_{I/P,n}^{\prime (k-1)}\boldsymbol{M}_{n}^H\boldsymbol{W}_{I/P}^{(i)})\\
			...
			\begin{equation}
				\begin{split}
					...
					& = \mathrm{Tr}\left(t_{P,0}^{\prime (k-1)}\boldsymbol{M}_{0}\boldsymbol{W}_{I,0}^{(i)}\right) + \mathrm{Tr}(t_{I,0}^{\prime (k-1)}\boldsymbol{M}_{0}\boldsymbol{W}_{P,0}^{(i)}) - t_{I,0}^{\prime (k-1)} t_{P,0}^{\prime (k-1)}
				\end{split}
			\end{equation}
			Therefore, we have
			\begin{equation}\label{eq:z_waveform_approx}
				\begin{split}
					\tilde{z}(\boldsymbol{W}_I^{(i)},\boldsymbol{W}_P^{(i)},\rho^{(i-1)})
					& = \mathrm{Tr}(\boldsymbol{A}_I^{(i)}\boldsymbol{W}_I^{(i)}) + \mathrm{Tr}(\boldsymbol{A}_P^{(i)}\boldsymbol{W}_P^{(i)}) \\
					& - \frac{3 k_4 (\rho^{(i-1)})^2 R_{\text{ant}}^2}{8} \sum_{n=-N+1}^{N-1} 2t_{I,n}^{\prime (k-1)} (t_{I,n}^{\prime (k-1)})^* + t_{P,n}^{\prime (k-1)} (t_{P,n}^{\prime (k-1)})^* \\
					& - \frac{3{k_4}{(\rho^{(i-1)})^2}{R_{\text{ant}}^2}}{2} t_{I,0}^{\prime (k-1)} t_{P,0}^{\prime (k-1)}
				\end{split}
			\end{equation}
			...
			\begin{equation}\label{eq:A_I,k}
				\begin{split}
					...
					& + \frac{3 k_4 (\rho^{(i-1)})^2 R_{\text{ant}}^2}{8} \sum_{n=-N+1}^{N-1} 2\left((t_{I,n}^{\prime (k-1)})^*\boldsymbol{M}_{n} + t_{I,n}^{\prime (k-1)}\boldsymbol{M}_{n}^H\right) \\
					& + \frac{3{k_4}{(\rho^{(i-1)})^2}{R_{\text{ant}}^2}}{2} t_{P,0}^{\prime (k-1)}\boldsymbol{M}_{0}
				\end{split}
			\end{equation}
			\begin{equation}\label{eq:A_P,k}
				\begin{split}
					\boldsymbol{A}_P^{(i)}
					& = \frac{k_2 \rho^{(i-1)} R_{\text{ant}}}{2}\boldsymbol{M}_0 \\
					& + \frac{3 k_4 (\rho^{(i-1)})^2 R_{\text{ant}}^2}{8} \sum_{n=-N+1}^{N-1} (t_{P,n}^{\prime (k-1)})^*\boldsymbol{M}_{n} + t_{P,n}^{\prime (k-1)}\boldsymbol{M}_{n}^H \\
					& + \frac{3{k_4}{(\rho^{(i-1)})^2}{R_{\text{ant}}^2}}{2} t_{I,0}^{\prime (k-1)}\boldsymbol{M}_{0}
				\end{split}
			\end{equation}
			...
			\begin{algorithm}
				...
				\begin{algorithmic}[1]
					...
					% \Until $\lVert \boldsymbol{W}_I^{(i)} - \boldsymbol{W}_I^{(k - 1)} \rVert_F / \lVert \boldsymbol{W}_I^{(i)} \rVert_F \le \epsilon$
