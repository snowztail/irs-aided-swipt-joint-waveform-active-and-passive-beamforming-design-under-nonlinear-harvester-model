...
\begin{document}
	...
	\begin{reviewersection}
		...
		\begin{response}
			...
			\end{equation}
			which consists of trace square and is not a semidefinite expression. Compared with standard SDP, problem (24) also introduces an additional log sum constraint (24b) for communication purpose, and the existing SDR bounds cannot be applied directly for performance guarantee.
			\label{re:1.5}
		\end{response}
	\end{reviewersection}
	...
	\begin{reviewersection}
		...
		\begin{response}
			In the single-user scenario, MRT is the global optimal information and power precoder. We decouple the waveform in the spatial and frequency domain by
			\begin{equation}
		...
		\begin{response}
			We agree that the proposed algorithm (especially Algorithm 2) could be very time-consuming for applications with short coherence time. The proposed algorithms ensure optimal performance by iteratively solving intricate optimization problems. Although suboptimal design in closed form (e.g. scaled matched filter) has been proved effective in practical WPT systems, the extension to SWIPT is much more challenging because the two waveforms and the splitting ratio need to be jointly designed under the influence of SNR.
